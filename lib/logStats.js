...
const createLogStats = ({
  logsDir,
  printerRegistry: initialPrinterRegistry = {},
  logStamp,
  errorLogStamp,
}) => {
  ...
  let printerRegistry = { ...initialPrinterRegistry };
  ...
  const syncPrinterRegistry = nextPrinterRegistry => {
    printerRegistry = { ...(nextPrinterRegistry || {}) };

    Object.keys(printerRegistry).forEach(printerId => {
      ensurePrinterStatsEntry(printerId);
    });
  };
