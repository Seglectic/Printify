  ...
  const runPoll = async () => {
    ...
    try {
      ...
      if (observedState.errorMessage) {
        ...
        // A missing binary is permanent, so stopping is right. Everything else
        // is usually temporary: at boot the service can beat udev to applying
        // the plugdev group, and libusb answers LIBUSB_ERROR_ACCESS for a
        // second or two. Latching off there left the printer invisible until
        // someone restarted the process, which no amount of replugging fixed.
        if (observedState.errorCode === 'ENOENT') {
          disablePlugin('could not find "ptouch-print"; plugin stopped to avoid stale state');
          return;
        }

        // Keep the UI honest about the printer dropping out, then keep polling
        // so it comes back on its own once the probe succeeds again.
