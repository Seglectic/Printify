...
const PREVIEWABLE_SOURCE_TYPES = new Set([
  'upload-pdf',
  'upload-pdf-xometry-pruned',
  'upload-image',
  'upload-image-bundled',
  'upload-zip-pdf',
]);

const PDF_PREVIEW_SOURCE_TYPES = new Set([
  'upload-pdf',
  'upload-pdf-xometry-pruned',
]);
...
const createPreviewer = ({
  ...
  const buildPreviewArgs = ({ sourceFilePath, sourceType, outputPath, thumbnail = false }) => {
    const inputPath = PDF_PREVIEW_SOURCE_TYPES.has(sourceType)
      ? `${sourceFilePath}[0]`
      : sourceFilePath;

    const args = [inputPath];

    if (PDF_PREVIEW_SOURCE_TYPES.has(sourceType)) {
