(function () {
  ...
  const createPrintifyConfirmSystem = ({
    ...
    const drawMark = (context, markType, progress, color) => {
      ...
      if (markType === 'error') {
        path.moveTo(errorPoints[0][0], errorPoints[0][1]);
        path.lineTo(errorPoints[1][0], errorPoints[1][1]);
        path.moveTo(errorPoints[2][0], errorPoints[2][1]);
        path.lineTo(errorPoints[3][0], errorPoints[3][1]);
      } else {
    ...
    const drawIndicator = (indicator, now) => {
      ...
      const isSettled = indicator.state === 'success' || indicator.state === 'error';
      ...
      const palette = getConfirmIndicatorPalette(indicator.state);
      ...
      const settleFillProgress = isSettled
        ? clamp((settledAge - 40) / 140, 0, 1)
        : 0;
      const markProgress = isSettled
        ? clamp((settledAge - 120) / 120, 0, 1)
        : 0;
      ...
      drawMark(
        context,
        indicator.state === 'error' ? 'error' : 'success',
        easeOutCubic(markProgress),
        palette.mark
      );
      context.restore();

      if (isSettled && settledAge >= 900 && indicator.state !== 'removing') {
