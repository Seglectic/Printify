...
const readClientVersionFromIndexScript = rootDir => {
  try {
    const indexScriptPath = path.join(rootDir, 'src', 'scripts', 'index.js');
    const indexScript = fs.readFileSync(indexScriptPath, 'utf8');
    const versionMatch = indexScript.match(/const APP_VERSION = ['"]([^'"]+)['"]/);
...
const registerRoutes = ({
  ...
  const getClientVersion = () => readClientVersionFromIndexScript(rootDir);
