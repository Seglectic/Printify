...
const supportedUiModes = new Set(['dropzone', 'label-builder']);
...
Object.entries(printers).forEach(([printerId, printerConfig]) => {
  ...
  if (printerConfig.uiMode && !supportedUiModes.has(printerConfig.uiMode)) {
    throw new Error(`Printer "${printerId}" has unsupported uiMode "${printerConfig.uiMode}"`);
  }

  if (printerConfig.enableLabelBuilder && !printerConfig.acceptedKinds.includes('image')) {
