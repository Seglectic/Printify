...
const DMG_STARTUP_MESSAGES = [
  'Game Boy launched in Japan in 1989.',
  'The original Game Boy screen is 160 by 144 pixels.',
  'The original Game Boy displays four shades of grey.',
  'Game Boy eventually sold over 100 million units.',
  'Nintendo credits Gunpei Yokoi as the mind behind the Game Boy.',
];

let activePopup = null;
let vendorLoadPromise = null;

const getRandomDmgMessage = () => (
  DMG_STARTUP_MESSAGES[Math.floor(Math.random() * DMG_STARTUP_MESSAGES.length)]
);
...
class PrintifyEmuPopup {
  ...
  async open() {
    ...
    if (!this.footerDrawer) {
      throw new Error('Game Boy plugin needs the footer drawer host.');
    }
    ...
    this.footerDrawer.registerSurface({
      ...
      tabLabel: 'DMG',
      eyebrow: '',
      statusLabel: 'Game Boy',
      height: '90vh',
      content: this.root,
      onVisibilityChange: isVisible => this.handleSurfaceVisibilityChange(isVisible),
    });
    this.footerDrawer.activateSurface(DMG_SURFACE_ID);
  ...
  announceStartupMessage() {
    ...
    this.showFeedback(getRandomDmgMessage(), { interruptAssistant: true });
  }
  ...
  renderShell() {
    ...
    this.root.innerHTML = `
      <section class="printify-dmg__window printify-dmg__window--embedded" aria-label="Game Boy">
        <div class="printify-dmg__launcher" data-role="dmg-launcher">
          <section class="printify-dmg__launcher-pane printify-dmg__launcher-pane--roms">
            <div class="printify-dmg__pane-header">
              <p class="printify-dmg__pane-eyebrow">ROMs</p>
            </div>
  ...
  handleSurfaceVisibilityChange(isVisible) {
  ...
  queueAutosave() {
    ...
    this.saveTimer = window.setTimeout(() => {
      this.saveTimer = null;
      this.persistSave(null, { silent: true });
    }, 900);
  }
  ...
  async persistSave(successMessage, options = {}) {
    if (!this.hasSaveSupport()) {
...
export const activatePlugin = async (pluginConfig, options = {}) => {
  ...
  try {
    ...
    options?.footerDrawer?.unregisterSurface?.(DMG_SURFACE_ID);
    activePopup = null;
    throw error;
  }
};
