...
const createPrintingService = ({
  ...
  const logPlannedTransport = (filePath, printerConfig, jobMeta = {}) => {
    ...
    switch (platformPrinterConfig?.printMode) {
      ...
      case 'cli': {
        const cliArgs = buildCliArgs(filePath, platformPrinterConfig, jobMeta);
  ...
  const runCliPrint = (filePath, printerConfig, done, jobMeta = {}) => {
    ...
    const cliArgs = buildCliArgs(filePath, printerConfig, jobMeta);
  ...
  const printImageItems = async ({ items, printerConfig, printCopies }) => {
    ...
    const shouldUseNativeTapeCopies = items.length === 1
      && printCopies > 1
      && supportsNativeTapeCopies(printerConfig);

    if (shouldUseNativeTapeCopies) {
      const primaryItem = items[0];
      const nativeCopyMeta = {
        ...primaryItem.jobMeta,
        originalFilename: formatCopiedJobName(primaryItem.file.originalname, printCopies),
        sourceType: 'upload-image-native-copies',
        totalCopies: printCopies,
        bundledSourceCount: printCopies,
        pages: printCopies,
        cliCopies: printCopies,
      };

      if (!printerConfig.monochrome) {
        await printPDF(primaryItem.file.path, printerConfig, {
          ...nativeCopyMeta,
      ...
      await converter
        .convertImgsToPng(
          [primaryItem.file.path],
          printerConfig,
          buildPreparedImagePath(primaryItem.file.path, printerConfig),
          nativeCopyMeta
        )
