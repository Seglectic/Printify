...
const createPrintingService = ({
  ...
  const recordPrintJob = (filePath, printerConfig, result, jobMeta = {}, extra = {}) => {
  ...
  const finishPrint = (filePath, printerConfig, jobMeta, transportResponse) => {
    logStamp('Printing file:', filePath, `to printer: ${printerConfig.driverName}`);
    serverSave.incrementPrintCounter();
    recordPrintJob(filePath, printerConfig, 'printed', jobMeta, { transportResponse });
  };
  ...
  const printPDF = (filePath, printerConfig, jobMetaOrCallback, callback) => {
    ...
    if (testing) {
      ...
      serverSave.incrementPrintCounter();
      recordPrintJob(filePath, printerConfig, 'skipped-testing', jobMeta);
    ...
    return printPromise
      .then(result => {
        finishPrint(filePath, printerConfig, jobMeta, result);
        return result;
      })
      .catch(error => {
        recordPrintJob(filePath, printerConfig, 'failed', jobMeta, { error });
        throw error;
      });
  };
