...
const createLogStore = ({
  ...
  const getRecentJobs = async ({ lookBackMinutes }) => {
    ...
    return Array.from(jobsByKey.values())
      ...
      .sort((leftJob, rightJob) => parseTimestamp(rightJob.timestamp) - parseTimestamp(leftJob.timestamp));
  };
  ...
  const findOriginalJob = async ({ chksum, beforeTimestamp = null }) => {
    ...
    const recentMatch = recentBuffer
      ...
      .sort((leftJob, rightJob) => parseTimestamp(rightJob.timestamp) - parseTimestamp(leftJob.timestamp))[0] || null;
