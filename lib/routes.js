...
const registerRoutes = ({
  ...
  app.post('/zebrapng', upload.single('pngFile'), (req, res) => {
    printingService.convertImgToPdf(req.file.path, printers.zebra);
    res.status(200).send('OK');
  });
  ...
  app.post('/brotherImg', upload.array('imgFile'), (req, res) => {
    printingService.convertImgToPdf(req.files[0].path, printers.brotherLaser);
    res.status(200).send('OK');
  });
  ...
  app.post('/dymopng', upload.single('pngFile'), (req, res) => {
    ...
    for (let index = 0; index < printCount; index += 1) {
      printingService.convertImgToPdf(filePath, printers.dymoLabel);
    }
