...
const createRuntimeConfig = () => {
  ...
  const ensurePluginConfigs = (pluginDefaultsById = {}) => {
    const document = readConfigDocument();
    const pluginsNode = document.get('plugins', true) || document.createNode({});
    let changed = false;
    ...
    Object.entries(pluginDefaultsById).forEach(([pluginId, defaultEntry]) => {
      ...
      if (!pluginsNode.has(pluginId)) {
        ...
        if (formattedHelpComment) {
          pluginNode.commentBefore = formattedHelpComment;
        }
      ...
      if (formattedHelpComment && pluginNode && !pluginNode.commentBefore) {
        pluginNode.commentBefore = formattedHelpComment;
        changed = true;
      }
    });
