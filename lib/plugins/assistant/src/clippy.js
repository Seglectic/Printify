...
clippy.Balloon.prototype = {
    ...
    MAX_CLOSE_BALLOON_DELAY:7600,
    READABLE_DELAY_PER_CHAR:42,
    ...
    estimateSpeechDuration:function (text) {
        var chars = String(text || '').split('');
        var time = Math.max(18, Math.round(this.WORD_SPEAK_TIME / 10));
        return chars.length * time;
    },

    getReadableDelay:function (text) {
        var minDelay = this.CLOSE_BALLOON_DELAY;
        var maxDelay = this.MAX_CLOSE_BALLOON_DELAY;
        var scaledDelay = minDelay + (String(text || '').length * this.READABLE_DELAY_PER_CHAR);
        return Math.min(maxDelay, Math.max(minDelay, scaledDelay));
    },
    ...
    _sayWords:function (text, hold, complete) {
        ...
        this._addWord = $.proxy(function () {
            ...
            if (idx > chars.length) {
                ...
                if (!this._hold) {
                    complete();
                    this.hide(false, this.getReadableDelay(text));
                }
            } else {
    ...
    pause:function () {
        window.clearTimeout(this._loop);
        if (this._hiding) {
            this._hideRemaining = Math.max(0, (this._hideDeadline || 0) - Date.now());
            this._clearHideTimer();
        }
    },
    ...
    resume:function () {
        ...
        if (!this._active && !this._hidden && this._hideRemaining) {
            this._scheduleHide(this._hideRemaining);
        }
    },

    _scheduleHide:function (delay) {
        var hideDelay = Math.max(0, delay || this.CLOSE_BALLOON_DELAY);
        this._clearHideTimer();
        this._hideRemaining = hideDelay;
        this._hideDeadline = Date.now() + hideDelay;
        this._hiding = window.setTimeout($.proxy(this._finishHideBalloon, this), hideDelay);
    },
    ...
    _clearHideTimer:function () {
        ...
        this._hideRemaining = null;
    }


};

clippy.BASE_PATH = '/plugins/assistant/agents/';
