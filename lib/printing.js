...
const createPrintingService = ({
  ...
  createFileChecksum, // Generates stable file hashes for logs and preview keys.
  createJobLogEntry,  // Shapes the shared metadata payload stored for each job.
  ...
  const prepareImageForPrinter = (filePath, printerConfig, outputImagePath) => (
  ...
  const printImageItems = async ({ items, printerConfig, printCopies }) => {
    ...
    if (cliPrefersPng) {
      if (items.length > 1 || printCopies > 1) {
        ...
        await converter
          .convertImgsToPng(bundledFiles, printerConfig, buildBundledImagePath(primaryItem.file.path, printerConfig))
      ...
      await Promise.all(items.flatMap(item => (
        Array.from({ length: printCopies }, (_, index) => {
          ...
          return converter
            .convertImgsToPng([item.file.path], printerConfig, buildPreparedImagePath(item.file.path, printerConfig))
            .then(outputImagePath => printPDF(outputImagePath, printerConfig, {
              ...jobMeta,
              checksumFilePath: outputImagePath,
              sourceFilePath: outputImagePath,
