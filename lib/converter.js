// ╭──────────────────────────╮
// │  converter.js            │
// │  ImageMagick helpers for │
// │  converting img files    │
// │  into printable PDFs     │
// ╰──────────────────────────╯
const fs       = require('fs');
const path     = require('path');
const { exec } = require('child_process');


// ┌──────────────────────┐
// │  Converter service   │
// └──────────────────────┘
const createConverter = ({
  imPath,
  logStamp,
  errorLogStamp,
}) => {
  const resolveMonochromeFlags = printerConfig => {
    const dither = String(printerConfig.monochromeDither || 'floydsteinberg').trim().toLowerCase();
    const monochromeBit = Number.parseInt(printerConfig.monochromeBit ?? 1, 10);

    if (monochromeBit > 1) {
      return [
        '-colorspace Gray',
        '-type Grayscale',
        `-depth ${monochromeBit}`,
      ];
    }

    if (dither === 'none') {
      return [
        '-colorspace Gray',
        '+dither',
        '-threshold 50%',
        '-type bilevel',
        '-depth 1',
      ];
    }

    return [
      '-colorspace Gray',
      `-dither ${dither === 'riemersma' ? 'Riemersma' : 'FloydSteinberg'}`,
      '-monochrome',
      '-type bilevel',
      '-depth 1',
      '-colors 2',
    ];
  };

  // Reuse the same sizing options for one-off image jobs and bundled pages so
  // printer-specific dimensions stay in one place.
  const buildImageConversionFlags = (printerConfig, options = {}) => {
    const flags = [];

    if (printerConfig.density) flags.push(`-density ${printerConfig.density}`);
    if (printerConfig.sizePx) flags.push(`-resize "${printerConfig.sizePx}"`);
    if (printerConfig.monochrome) {
      flags.push('-background "white"');
      flags.push('-alpha remove');
      flags.push('-alpha off');
      flags.push(...resolveMonochromeFlags(printerConfig));

      if (options.invertPrint && Number.parseInt(printerConfig.monochromeBit ?? 1, 10) === 1) {
        flags.push('-negate');
      }
    }

    return flags.join(' ');
  };

  // Keep generated bundle names readable because operators inspect uploads
  // directly when diagnosing print issues on shared machines.
  const buildBundledPdfPath = (imgFilePaths, pdfFilePath) => {
    if (pdfFilePath) return pdfFilePath;

    const firstFile = imgFilePaths[0];
    const firstName = path.parse(firstFile).name;
    return path.join(path.dirname(firstFile), `${firstName}-bundle-${Date.now()}.pdf`);
  };

  const buildBundledImagePath = (imgFilePaths, imageFilePath) => {
    if (imageFilePath) return imageFilePath;

    const firstFile = imgFilePaths[0];
    const firstName = path.parse(firstFile).name;
    return path.join(path.dirname(firstFile), `${firstName}-bundle-${Date.now()}.png`);
  };

  // ┌────────────────────┐
  // │  Convert to PDF    │
  // └────────────────────┘
  const convertImgsToPdf = (imgFilePaths, printerConfig, pdfFilePath, options = {}) => {
    if (!Array.isArray(imgFilePaths) || imgFilePaths.length === 0) {
      errorLogStamp('No input image files were provided.');
      return Promise.reject(new Error('No input image files were provided.'));
    }

    const missingInput = imgFilePaths.find(imgFilePath => !fs.existsSync(imgFilePath));

    if (missingInput) {
      errorLogStamp('Input img file does not exist.');
      return Promise.reject(new Error(`Input img file does not exist: ${missingInput}`));
    }

    const outputPdfPath = buildBundledPdfPath(imgFilePaths, pdfFilePath);
    const inputArgs = imgFilePaths.map(imgFilePath => `"${imgFilePath}"`).join(' ');
    const conversionFlags = buildImageConversionFlags(printerConfig, options);
    const command = `"${imPath}" ${inputArgs}${conversionFlags ? ` ${conversionFlags}` : ''} -format "pdf" "${outputPdfPath}"`;

    return new Promise((resolve, reject) => {
      exec(command, (error, stdout, stderr) => {
        if (error) {
          errorLogStamp(`ImageMagick error: ${error.message}`);
          reject(error);
          return;
        }

        if (stderr) {
          errorLogStamp(`ImageMagick stderr: ${stderr}`);
        }

        // logStamp(`ImageMagick command executed successfully. Output: ${stdout}`);
        resolve(outputPdfPath);
      });
    });
  };

  const convertImgToPdf = (imgFilePath, printerConfig, pdfFilePath, options = {}) => (
    convertImgsToPdf([imgFilePath], printerConfig, pdfFilePath || `${imgFilePath}.pdf`, options)
  );

  const convertImgsToPng = (imgFilePaths, printerConfig, imageFilePath, options = {}) => {
    if (!Array.isArray(imgFilePaths) || imgFilePaths.length === 0) {
      errorLogStamp('No input image files were provided.');
      return Promise.reject(new Error('No input image files were provided.'));
    }

    const missingInput = imgFilePaths.find(imgFilePath => !fs.existsSync(imgFilePath));

    if (missingInput) {
      errorLogStamp('Input img file does not exist.');
      return Promise.reject(new Error(`Input img file does not exist: ${missingInput}`));
    }

    const outputImagePath = buildBundledImagePath(imgFilePaths, imageFilePath);
    const inputArgs = imgFilePaths.map(imgFilePath => `"${imgFilePath}"`).join(' ');
    const conversionFlags = buildImageConversionFlags(printerConfig, options);
    const command = `"${imPath}" ${inputArgs}${conversionFlags ? ` ${conversionFlags}` : ''} -background "white" +append "${outputImagePath}"`;

    return new Promise((resolve, reject) => {
      exec(command, (error, stdout, stderr) => {
        if (error) {
          errorLogStamp(`ImageMagick error: ${error.message}`);
          reject(error);
          return;
        }

        if (stderr) {
          errorLogStamp(`ImageMagick stderr: ${stderr}`);
        }

        logStamp(`ImageMagick command executed successfully. Output: ${stdout}`);
        resolve(outputImagePath);
      });
    });
  };

  return {
    convertImgsToPdf,
    convertImgToPdf,
    convertImgsToPng,
  };
};

module.exports = {
  createConverter,
};
