// ╭──────────────────────────╮
// │  ingest.js               │
// │  Upload ingest and       │
// │  duplicate session flow  │
// ╰──────────────────────────╯
const fs = require('fs');
const path = require('path');
const crypto = require('crypto');
const multer = require('multer');
const yauzl = require('yauzl');
const { resolveLoggedPath } = require('./filePaths');

const SESSION_TTL_MS = 15 * 60 * 1000;
const SUPPORTED_IMAGE_EXTENSIONS = new Set(['.png', '.jpg', '.jpeg', '.tif', '.tiff', '.webp']);
const SUPPORTED_IMAGE_MIME_TYPES = new Set([
  'image/png',
  'image/jpeg',
  'image/jpg',
  'image/tiff',
  'image/webp',
]);

class IngestValidationError extends Error {
  constructor(message) {
    super(message);
    this.name = 'IngestValidationError';
    this.code = 'INGEST_VALIDATION_FAILED';
    this.status = 400;
  }
}


// ┌──────────────────────┐
// │  Path helpers        │
// └──────────────────────┘
const normalizeRelativePath = value => {
  const normalized = path.posix.normalize(`/${String(value || '').replace(/\\/g, '/')}`);
  return normalized.replace(/^\/+/, '');
};

const buildSafeExtractedPath = (baseDir, entryName) => {
  const relativePath = normalizeRelativePath(entryName)
    .split('/')
    .filter(segment => segment && segment !== '.' && segment !== '..')
    .join('/');

  return path.join(baseDir, relativePath);
};

const crockfordAlphabet = '0123456789ABCDEFGHJKMNPQRSTVWXYZ';
let lastStampTick = 0n;
let lastStampCounter = 0n;

const createCrockfordStamp = () => {
  const epochMicros = BigInt(Date.now()) * 1000n;
  const hrMicros = process.hrtime.bigint() / 1000n;
  let tick = epochMicros + (hrMicros % 1000n);

  if (tick <= lastStampTick) {
    lastStampCounter += 1n;
    tick = lastStampTick + lastStampCounter;
  } else {
    lastStampTick = tick;
    lastStampCounter = 0n;
    lastStampTick = tick;
  }

  let epoch = tick;
  let out = '';

  do {
    out = crockfordAlphabet[Number(epoch % 32n)] + out;
    epoch /= 32n;
  } while (epoch > 0n);

  return out;
};

const createSessionId = () => crypto.randomBytes(12).toString('hex');
const createItemId = () => crypto.randomBytes(10).toString('hex');

const removeFileIfPresent = async filePath => {
  if (!filePath) return;

  try {
    await fs.promises.unlink(filePath);
  } catch (error) {
    if (error.code !== 'ENOENT') {
      throw error;
    }
  }
};

const removeEmptyDirectories = async (startDir, stopDir) => {
  let currentDir = startDir ? path.resolve(startDir) : '';
  const resolvedStopDir = stopDir ? path.resolve(stopDir) : '';

  while (currentDir && currentDir.startsWith(resolvedStopDir) && currentDir !== resolvedStopDir) {
    try {
      const entries = await fs.promises.readdir(currentDir);
      if (entries.length > 0) {
        return;
      }

      await fs.promises.rmdir(currentDir);
      currentDir = path.dirname(currentDir);
    } catch (error) {
      if (error.code === 'ENOENT') {
        currentDir = path.dirname(currentDir);
        continue;
      }

      if (error.code === 'ENOTEMPTY') {
        return;
      }

      throw error;
    }
  }
};

const removeStagedFile = async (filePath, uploadsDir, errorLogStamp, messagePrefix) => {
  if (!filePath) return;

  try {
    await removeFileIfPresent(filePath);
    await removeEmptyDirectories(path.dirname(filePath), uploadsDir);
  } catch (error) {
    errorLogStamp(`${messagePrefix} ${filePath}:`, error.message);
  }
};

const isSupportedImageFile = file => {
  const extension = path.extname(file?.originalname || file?.filename || '').toLowerCase();
  const mimeType = String(file?.mimetype || '').trim().toLowerCase();

  return SUPPORTED_IMAGE_EXTENSIONS.has(extension) || SUPPORTED_IMAGE_MIME_TYPES.has(mimeType);
};

const getUploadsStagingDir = uploadsDir => path.join(uploadsDir, 'staging');

const isStagedUploadPath = (filePath, uploadsDir) => {
  const stagingDir = path.resolve(getUploadsStagingDir(uploadsDir));
  const resolvedFilePath = path.resolve(filePath || '');
  return resolvedFilePath.startsWith(`${stagingDir}${path.sep}`);
};

const moveItemIntoPrinterFolder = async ({ item, printerId, uploadsDir }) => {
  const originalPath = item?.file?.path;

  if (!originalPath || !isStagedUploadPath(originalPath, uploadsDir)) {
    return item;
  }

  const stagingDir = path.resolve(getUploadsStagingDir(uploadsDir));
  const relativePath = path.relative(stagingDir, path.resolve(originalPath));
  const pathSegments = relativePath.split(path.sep).filter(Boolean);
  const targetPath = path.join(uploadsDir, printerId, ...pathSegments);

  await fs.promises.mkdir(path.dirname(targetPath), { recursive: true });
  await fs.promises.rename(originalPath, targetPath);
  await removeEmptyDirectories(path.dirname(originalPath), stagingDir);

  return {
    ...item,
    file: {
      ...item.file,
      path: targetPath,
      filename: path.basename(targetPath),
    },
    jobMeta: {
      ...item.jobMeta,
      ...(item.jobMeta?.checksumFilePath === originalPath
        ? { checksumFilePath: targetPath }
        : {}),
    },
  };
};

const getUploadStageDir = req => {
  if (!req.printifyUploadStageDir) {
    req.printifyUploadStageDir = createCrockfordStamp();
  }

  return req.printifyUploadStageDir;
};

// ┌──────────────────────┐
// │  Ingest service      │
// └──────────────────────┘
const createIngestService = ({
  uploadsDir,
  printingService,
  deduplicator,
  inspectUpload = null,
  resolvePendingUploadAction = null,
  logStamp,
  errorLogStamp,
}) => {
  const pendingSessions = new Map();
  const uploadStorage = multer.diskStorage({
    destination: (req, file, cb) => {
      const stageDir = getUploadStageDir(req);
      const destinationDir = path.join(getUploadsStagingDir(uploadsDir), stageDir);

      fs.mkdirSync(destinationDir, { recursive: true });
      cb(null, destinationDir);
    },
    filename: (req, file, cb) => {
      cb(null, path.basename(file.originalname));
    },
  });
  const upload = multer({ storage: uploadStorage });

  const clearSessionTimeout = session => {
    if (session?.timeout) {
      clearTimeout(session.timeout);
      session.timeout = null;
    }
  };

  const cleanupPendingSessionItems = session => (
    Promise.all([...(session?.items || []), ...(session?.deferredPluginItems || [])].map(item => (
      removeStagedFile(
        item?.file?.path,
        uploadsDir,
        errorLogStamp,
        'Pending ingest cleanup failed for'
      )
    )))
  );

  const scheduleSessionExpiry = sessionId => {
    const session = pendingSessions.get(sessionId);

    if (!session) {
      return;
    }

    session.expiresAt = Date.now() + SESSION_TTL_MS;
    session.timeout = setTimeout(async () => {
      const expiringSession = pendingSessions.get(sessionId);
      if (!expiringSession) {
        return;
      }

      pendingSessions.delete(sessionId);
      await cleanupPendingSessionItems(expiringSession);
    }, SESSION_TTL_MS);
  };

  const createJobMeta = ({
    requestPath,
    file,
    sourceType,
    extra = {},
  }) => ({
    originalFilename: file.originalname,
    fileSizeBytes: Number.isFinite(file.size) ? file.size : null,
    sourceRoute: requestPath,
    sourceType,
    ...extra,
  });

  const extractZipEntries = async ({
    zipFile,
    uploadJobMeta = {},
    printerId,
    requestPath,
  }) => {
    const zipBaseName = path.parse(zipFile.filename).name;
    const extractionDir = path.join(uploadsDir, printerId, 'extracted', `${Date.now()}-${zipBaseName}`);
    const extractedEntries = [];

    await fs.promises.mkdir(extractionDir, { recursive: true });

    await new Promise((resolve, reject) => {
      yauzl.open(zipFile.path, { lazyEntries: true }, (openError, zipHandle) => {
        if (openError) {
          reject(openError);
          return;
        }

        zipHandle.readEntry();

        zipHandle.on('entry', entry => {
          if (/\/$/.test(entry.fileName)) {
            zipHandle.readEntry();
            return;
          }

          if (path.extname(entry.fileName).toLowerCase() !== '.pdf') {
            zipHandle.readEntry();
            return;
          }

          const extractedPath = buildSafeExtractedPath(extractionDir, entry.fileName);

          zipHandle.openReadStream(entry, (streamError, readStream) => {
            if (streamError) {
              reject(streamError);
              return;
            }

            fs.mkdirSync(path.dirname(extractedPath), { recursive: true });
            const writeStream = fs.createWriteStream(extractedPath);
            readStream.pipe(writeStream);

            writeStream.on('close', () => {
              const extractedFileSize = fs.existsSync(extractedPath)
                ? fs.statSync(extractedPath).size
                : null;
              const extractedIndex = extractedEntries.length + 1;
              const uploadGroupId = String(uploadJobMeta?.uploadGroupId || uploadJobMeta?.clientJobId || '').trim() || null;
              const extractedClientJobId = uploadGroupId
                ? `${uploadGroupId}-entry-${extractedIndex}`
                : null;

              extractedEntries.push({
                file: {
                  path: extractedPath,
                  filename: path.basename(extractedPath),
                  originalname: entry.fileName,
                  size: extractedFileSize,
                },
                jobMeta: createJobMeta({
                  requestPath,
                  file: {
                    filename: path.basename(extractedPath),
                    originalname: entry.fileName,
                    size: extractedFileSize,
                  },
                  sourceType: 'upload-zip-pdf',
                  extra: {
                    ...(uploadGroupId ? { uploadGroupId } : {}),
                    ...(extractedClientJobId ? { clientJobId: extractedClientJobId } : {}),
                    checksumFilePath: extractedPath,
                    sourceArchiveName: zipFile.originalname,
                  },
                }),
              });
              zipHandle.readEntry();
            });

            writeStream.on('error', reject);
            readStream.on('error', reject);
          });
        });

        zipHandle.on('end', resolve);
        zipHandle.on('error', reject);
      });
    });

    await removeFileIfPresent(zipFile.path);

    if (!extractedEntries.length) {
      throw new IngestValidationError(
        `ZIP "${zipFile.originalname}" did not contain any printable PDF or image files.`
      );
    }

    return extractedEntries;
  };

  const buildUploadItems = async ({
    requestPath,
    printerId,
    fileKind,
    uploadedFiles,
    uploadedJobMetaList,
  }) => {
    if (fileKind === 'zip') {
      const extractedGroups = await Promise.all(uploadedFiles.map((file, index) => (
        extractZipEntries({
          zipFile: file,
          uploadJobMeta: uploadedJobMetaList[index] || {},
          printerId,
          requestPath,
        })
      )));

      return extractedGroups.flat();
    }

    const sourceType = fileKind === 'pdf' ? 'upload-pdf' : 'upload-image';

    return uploadedFiles.map((file, index) => {
      if (fileKind === 'image' && !isSupportedImageFile(file)) {
        throw new IngestValidationError(
          `Unsupported image upload "${file.originalname}". Use PNG, JPG, TIFF, or WEBP.`
        );
      }

      return {
        file,
        jobMeta: createJobMeta({
          requestPath,
          file,
          sourceType,
          extra: {
            ...(uploadedJobMetaList[index] || {}),
            ...(fileKind === 'image' ? { checksumFilePath: file.path } : {}),
          },
        }),
      };
    });
  };

  const enrichItemsWithDuplicateChecks = async ({ items, printerConfig }) => {
    return Promise.all(items.map(async item => {
      const checksumFilePath = item.jobMeta.checksumFilePath || item.file.path;
      const checksum = checksumFilePath
        ? await printingService.createFileChecksum(checksumFilePath)
        : null;
      const duplicateJob = checksum
        ? await deduplicator.findRecentDuplicate({
          chksum: checksum,
          printerId: printerConfig.id,
        })
        : null;

      return {
        ...item,
        checksum,
        duplicateJob,
      };
    }));
  };

  const markItemsAsReprints = items => items.map(item => ({
    ...item,
    jobMeta: {
      ...item.jobMeta,
      isReprint: true,
      reprintSourceTimestamp: item.duplicateJob?.timestamp || null,
    },
  }));

  const cleanupDuplicateItems = items => (
    Promise.all(items.map(item => (
      removeStagedFile(
        item?.file?.path,
        uploadsDir,
        errorLogStamp,
        'Duplicate upload cleanup failed for'
      )
    )))
  );

  const canReuseDuplicateSource = item => {
    const duplicateSourcePath = resolveLoggedPath(item?.duplicateJob?.filePath);
    return Boolean(duplicateSourcePath) && fs.existsSync(duplicateSourcePath);
  };

  const buildDuplicatePayload = ({ item, printerConfig }) => ({
    id: item.id,
    checksum: item.checksum || null,
    originalFilename: item.jobMeta.originalFilename || item.file.originalname,
    printerId: printerConfig.id,
    duplicateTimestamp: item.duplicateJob?.timestamp || null,
    duplicatePrinterName: item.duplicateJob?.printerName || printerConfig.driverName || null,
    sourceType: item.jobMeta.sourceType || null,
    sourceArchiveName: item.jobMeta.sourceArchiveName || null,
    uploadGroupId: item.jobMeta.uploadGroupId || null,
  });

  const buildPluginActionSummary = pendingItems => {
    const sampleIntervention = pendingItems[0]?.pluginIntervention || {};
    const prompt = sampleIntervention.prompt || {};
    const wastePageCount = pendingItems.reduce((total, item) => (
      total + Number(item?.pluginIntervention?.wastePageCount || 0)
    ), 0);
    const keepPageCount = pendingItems.reduce((total, item) => (
      total + Number(item?.pluginIntervention?.keepPageCount || 0)
    ), 0);
    const fileCount = pendingItems.length;
    const wastePageLabel = `${wastePageCount} waste page${wastePageCount === 1 ? '' : 's'}`;
    const documentLabel = `${fileCount} packing slip${fileCount === 1 ? '' : 's'}`;

    return {
      pluginId: sampleIntervention.pluginId || null,
      eyebrow: prompt.eyebrow || 'Packing Slip Check',
      title: prompt.title || 'Xometry - Packing Slip Waste Detected',
      message: fileCount === 1
        ? (prompt.message || `${wastePageLabel} contain only zero-quantity part sheets. Prune them before printing?`)
        : `${wastePageLabel} were found across ${documentLabel}. Prune them before printing?`,
      subtext: fileCount === 1
        ? (prompt.subtext || '')
        : `Keeping ${keepPageCount} printable page${keepPageCount === 1 ? '' : 's'} across ${documentLabel}.`,
      logoUrl: prompt.logoUrl || null,
      confirmLabel: prompt.confirmLabel || 'Prune',
      secondaryLabel: prompt.secondaryLabel || 'Print Original',
      cancelLabel: prompt.cancelLabel || 'Cancel',
      wastePageCount,
      keepPageCount,
      fileCount,
    };
  };

  const stagePendingDuplicates = ({
    pendingItems,
    fileKind,
    printerConfig,
    requestBody = {},
    deferredPluginItems = [],
  }) => {
    if (!pendingItems.length) {
      return null;
    }

    const sessionId = createSessionId();
    const session = {
      id: sessionId,
      type: 'duplicates',
      fileKind,
      printerConfig,
      requestBody,
      items: pendingItems.map(item => ({
        ...item,
        id: createItemId(),
      })),
      deferredPluginItems: deferredPluginItems.map(item => ({
        ...item,
        id: createItemId(),
      })),
      createdAt: Date.now(),
      expiresAt: Date.now() + SESSION_TTL_MS,
      timeout: null,
    };

    pendingSessions.set(sessionId, session);
    scheduleSessionExpiry(sessionId);

    return {
      sessionId,
      duplicates: session.items.map(item => buildDuplicatePayload({ item, printerConfig })),
    };
  };

  const stagePendingPluginAction = ({
    pendingItems,
    fileKind,
    printerConfig,
    requestBody = {},
  }) => {
    if (!pendingItems.length) {
      return null;
    }

    const sessionId = createSessionId();
    const sessionItems = pendingItems.map(item => ({
      ...item,
      id: createItemId(),
    }));
    const session = {
      id: sessionId,
      type: 'plugin-action',
      fileKind,
      printerConfig,
      requestBody,
      items: sessionItems,
      createdAt: Date.now(),
      expiresAt: Date.now() + SESSION_TTL_MS,
      timeout: null,
    };

    pendingSessions.set(sessionId, session);
    scheduleSessionExpiry(sessionId);

    return {
      sessionId,
      ...buildPluginActionSummary(sessionItems),
    };
  };

  const processUpload = async ({
    requestPath,
    printerConfig,
    fileKind,
    uploadedFiles,
    uploadedJobMetaList = [],
    requestBody = {},
  }) => {
    const uploadItems = await buildUploadItems({
      requestPath,
      printerId: printerConfig.id,
      fileKind,
      uploadedFiles,
      uploadedJobMetaList,
    });
    const scannedItems = await enrichItemsWithDuplicateChecks({
      items: uploadItems,
      printerConfig,
    });
    const normalizedFileKind = fileKind === 'zip' ? 'pdf' : fileKind;
    const normalizedItems = scannedItems.map(item => ({
      ...item,
      jobMeta: {
        ...item.jobMeta,
        ...(item.checksum ? { chksum: item.checksum } : {}),
      },
    }));
    const sortedItems = await Promise.all(normalizedItems.map(item => (
      moveItemIntoPrinterFolder({
        item,
        printerId: printerConfig.id,
        uploadsDir,
      })
    )));
    const printableReadyItems = [];
    const pendingPluginItems = [];
    const pendingDuplicateItems = [];

    for (const item of sortedItems) {
      const pluginIntervention = normalizedFileKind === 'pdf' && typeof inspectUpload === 'function'
        ? await inspectUpload({
          item,
          fileKind: normalizedFileKind,
          printerConfig,
          requestBody,
        })
        : null;

      const itemWithIntervention = pluginIntervention
        ? {
          ...item,
          pluginIntervention,
        }
        : item;

      if (item.duplicateJob) {
        if (!pluginIntervention && canReuseDuplicateSource(itemWithIntervention)) {
          await removeStagedFile(
            itemWithIntervention.file.path,
            uploadsDir,
            errorLogStamp,
            'Duplicate upload cleanup failed for'
          );
        }

        pendingDuplicateItems.push(itemWithIntervention);
        continue;
      }

      if (pluginIntervention) {
        pendingPluginItems.push(itemWithIntervention);
        continue;
      }

      printableReadyItems.push(item);
    }

    const printedCount = await printingService.printStagedItems({
      items: printableReadyItems,
      fileKind: normalizedFileKind,
      printerConfig,
      requestBody,
    });
    const pendingDuplicateSession = stagePendingDuplicates({
      pendingItems: pendingDuplicateItems,
      fileKind: normalizedFileKind,
      printerConfig,
      requestBody,
      deferredPluginItems: pendingPluginItems,
    });
    const pendingPluginSession = pendingDuplicateSession
      ? null
      : stagePendingPluginAction({
        pendingItems: pendingPluginItems,
        fileKind: normalizedFileKind,
        printerConfig,
        requestBody,
      });

    return {
      ok: true,
      printedCount,
      skippedCount: 0,
      skippedDuplicates: [],
      ...(pendingDuplicateSession ? {
        needsConfirmation: true,
        sessionId: pendingDuplicateSession.sessionId,
        duplicates: pendingDuplicateSession.duplicates,
      } : {}),
      ...(pendingPluginSession ? {
        pendingPluginAction: pendingPluginSession,
      } : {}),
    };
  };

  const confirmPendingDuplicates = async ({
    sessionId,
    approvedItemIds = [],
  }) => {
    const session = pendingSessions.get(sessionId);

    if (!session) {
      const error = new Error('Pending ingest session not found');
      error.code = 'SESSION_NOT_FOUND';
      throw error;
    }

    if (session.type !== 'duplicates') {
      const error = new Error('Pending ingest session type mismatch');
      error.code = 'SESSION_NOT_FOUND';
      throw error;
    }

    clearSessionTimeout(session);
    pendingSessions.delete(sessionId);

    const approvedItemIdSet = new Set(approvedItemIds.map(value => String(value)));
    const approvedItems = session.items.filter(item => approvedItemIdSet.has(item.id));
    const declinedItems = session.items.filter(item => !approvedItemIdSet.has(item.id));
    const approvedReprintItems = markItemsAsReprints(approvedItems);
    const pluginPendingItems = [
      ...(Array.isArray(session.deferredPluginItems) ? session.deferredPluginItems : []),
      ...approvedReprintItems.filter(item => item.pluginIntervention),
    ];
    const immediatelyPrintableApprovedItems = approvedReprintItems.filter(item => !item.pluginIntervention);
    const reusableApprovedItems = immediatelyPrintableApprovedItems.filter(canReuseDuplicateSource);
    const stagedApprovedItems = await Promise.all(
      immediatelyPrintableApprovedItems
        .filter(item => !canReuseDuplicateSource(item))
        .map(item => moveItemIntoPrinterFolder({
          item,
          printerId: session.printerConfig.id,
          uploadsDir,
        }))
    );

    await Promise.all(declinedItems.map(item => (
      removeStagedFile(
        item.file.path,
        uploadsDir,
        errorLogStamp,
        'Declined ingest cleanup failed for'
      )
    )));

    const reuseResults = await Promise.all(reusableApprovedItems.map(item => (
      (() => {
        logStamp(
          '[id-debug] duplicate confirm reuse',
          `printer=${session.printerConfig.id}`,
          `clientJobId=${item.jobMeta?.clientJobId || 'none'}`,
          `duplicateJobId=${item.duplicateJob?.jobId || 'none'}`,
          `itemId=${item.id}`
        );

        return printingService.reprintLoggedJob({
          printJob: item.duplicateJob,
          printerConfig: session.printerConfig,
          fileKind: session.fileKind,
          requestBody: session.requestBody || {},
          sourceRoute: '/ingest/confirm',
          jobMeta: {
            clientJobId: item.jobMeta?.clientJobId || null,
            uploadGroupId: item.jobMeta?.uploadGroupId || null,
          },
        });
      })()
    )));
    const stagedPrintedCount = await printingService.printStagedItems({
      items: stagedApprovedItems,
      fileKind: session.fileKind,
      printerConfig: session.printerConfig,
      requestBody: session.requestBody || {},
    });
    await cleanupDuplicateItems(stagedApprovedItems);
    const printedCount = reuseResults.reduce((total, count) => total + Number(count || 0), 0) + stagedPrintedCount;
    const pendingPluginSession = stagePendingPluginAction({
      pendingItems: pluginPendingItems,
      fileKind: session.fileKind,
      printerConfig: session.printerConfig,
      requestBody: session.requestBody || {},
    });

    return {
      ok: true,
      printedCount,
      skippedCount: declinedItems.length,
      skippedDuplicates: declinedItems.map(item => ({
        checksum: item.checksum || null,
        originalFilename: item.jobMeta.originalFilename || item.file.originalname,
        printerId: session.printerConfig.id,
      })),
      ...(pendingPluginSession ? {
        pendingPluginAction: pendingPluginSession,
      } : {}),
    };
  };

  const confirmPendingPluginAction = async ({
    sessionId,
    action,
  }) => {
    const session = pendingSessions.get(sessionId);

    if (!session) {
      const error = new Error('Pending ingest session not found');
      error.code = 'SESSION_NOT_FOUND';
      throw error;
    }

    if (session.type !== 'plugin-action') {
      const error = new Error('Pending ingest session type mismatch');
      error.code = 'SESSION_NOT_FOUND';
      throw error;
    }

    clearSessionTimeout(session);
    pendingSessions.delete(sessionId);

    const normalizedAction = String(action || '').trim().toLowerCase();

    if (!['prune', 'original', 'cancel'].includes(normalizedAction)) {
      const error = new Error('Unsupported pending plugin action');
      error.code = 'INVALID_PLUGIN_ACTION';
      error.status = 400;
      throw error;
    }

    if (normalizedAction === 'cancel') {
      await cleanupPendingSessionItems(session);
      return {
        ok: true,
        canceled: true,
        printedCount: 0,
        skippedCount: 0,
        skippedDuplicates: [],
      };
    }

    if (typeof resolvePendingUploadAction !== 'function') {
      const error = new Error('Pending upload action handler is unavailable');
      error.code = 'PLUGIN_ACTION_UNAVAILABLE';
      throw error;
    }

    const resolvedItems = [];
    const cleanupAfterSuccessPaths = [];

    for (const item of session.items) {
      const resolution = await resolvePendingUploadAction({
        pluginId: item.pluginIntervention?.pluginId,
        action: normalizedAction,
        item,
        intervention: item.pluginIntervention,
        printerConfig: session.printerConfig,
        requestBody: session.requestBody || {},
      });

      if (Array.isArray(resolution?.cleanupAfterSuccessPaths)) {
        cleanupAfterSuccessPaths.push(...resolution.cleanupAfterSuccessPaths);
      }

      if (resolution?.item) {
        resolvedItems.push(resolution.item);
      }
    }

    const printedCount = await printingService.printStagedItems({
      items: resolvedItems,
      fileKind: session.fileKind,
      printerConfig: session.printerConfig,
      requestBody: session.requestBody || {},
    });

    await Promise.all(cleanupAfterSuccessPaths.map(filePath => (
      removeStagedFile(
        filePath,
        uploadsDir,
        errorLogStamp,
        'Plugin action cleanup failed for'
      )
    )));

    return {
      ok: true,
      printedCount,
      skippedCount: 0,
      skippedDuplicates: [],
    };
  };

  const purgePendingSessions = async () => {
    const sessions = Array.from(pendingSessions.values());

    sessions.forEach(session => {
      clearSessionTimeout(session);
    });
    pendingSessions.clear();

    await Promise.all(sessions.map(session => cleanupPendingSessionItems(session)));
  };

  return {
    createUploadMiddleware: ({ fieldName, isMulti }) => (
      isMulti ? upload.array(fieldName) : upload.single(fieldName)
    ),
    processUpload,
    confirmPendingDuplicates,
    confirmPendingPluginAction,
    purgePendingSessions,
  };
};

module.exports = {
  createIngestService,
};
