...
const createPrintingService = ({
  ...
  const resolvePlatformPrinterConfig = printerConfig => {
    ...
    return {
      ...printerConfig,
      driverName: printerConfig.linuxDriverName || printerConfig.driverName,
      printMode: printerConfig.linuxPrintMode || printerConfig.printMode,
    };
  };
  ...
  const buildBundledPdfPath = (sourceFilePath, printerConfig) => (
    path.join(
      ...
      `${path.parse(sourceFilePath).name}-${printerConfig.id || 'printer'}-bundle-${Date.now()}.pdf`
    )
  );
    ...
    return extractionPromise
      .then(entries => {
        const printPromises = entries.map(entry => printPDF(entry.filePath, printerConfig, {
          ...
          checksumFilePath: entry.filePath,
