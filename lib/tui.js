...
const createTui = ({
  ...
  const showHelp = () => {
    ...
    logStamp('  reload                    Re-read config.yaml into runtime state');
    ...
    switch (command.toLowerCase()) {
      ...
      case 'reload': {
        ...
        if (result.changes.length === 0) {
          logStamp('Reloaded config.yaml with no runtime changes.');
        } else {
  ...
  try {
    while (true) {
      ...
      try {
        ...
        logStamp(`Saved port ${change.value} to config.yaml.`);
        return change.value;
      } catch (error) {
