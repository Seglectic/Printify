...
const registerRoutes = ({
  ...
  const removeUploadedFile = async filePath => {
    ...
    try {
      ...
      if (error.code !== 'ENOENT') {
        errorLogStamp(`Upload cleanup failed for ${filePath}:`, error.message);
      }
    }
  };

  const findRecentDuplicateJob = async (fileKind, filePath) => {
    if (fileKind === 'zip' || !filePath) {
      return {
        checksum: null,
        duplicateJob: null,
      };
    ...
    const checksum = await printingService.createFileChecksum(filePath);
    const duplicateJob = checksum
      ? await logStore.findRecentJobByChecksum({
        chksum: checksum,
        lookBackMinutes: DUPLICATE_LOOKBACK_MINUTES,
      })
      : null;
  ...
  const printUploadedImages = (req, uploadedImages, printerConfig, printCopies) => {
    if (printerConfig.bundleCopies && (uploadedImages.length > 1 || printCopies > 1)) {
      const bundledFiles = uploadedImages.flatMap(({ file }) => (
        Array.from({ length: printCopies }, () => file.path)
      ));
      const primaryJobMeta = uploadedImages[0]?.jobMeta || {};

      return printingService.printBundledImages(bundledFiles, printerConfig, {
        originalFilename: uploadedImages.length === 1
          ? `${uploadedImages[0].file.originalname} x${printCopies}`
          : `${uploadedImages.length} image files bundled`,
        checksumFilePath: uploadedImages[0].file.path,
        sourceRoute: req.path,
    ...
    const jobs = uploadedImages.flatMap(({ file, jobMeta }) => Array.from({ length: printCopies }, (_, index) => ({
      file,
      jobMeta,
    ...
    return Promise.all(jobs.map(({ file, jobMeta, copyIndex, totalCopies }) => printUploadedFile(
      req,
      file,
      printerConfig,
      'image',
  ...
  const registerPrinterUploadRoute = (printerId, printerConfig, fileKind, mode) => {
    ...
    app.post(routePath, middleware, async (req, res) => {
      ...
      try {
        const duplicateScanResults = await Promise.all(uploadedFiles.map(file => (
          findRecentDuplicateJob(fileKind, file.path)
        )));
        const filesToPrint = [];
        ...
        for (let index = 0; index < uploadedFiles.length; index += 1) {
          ...
          const duplicateScan = duplicateScanResults[index] || {};
          const duplicateJob = duplicateScan.duplicateJob || null;

          if (duplicateJob) {
            skippedDuplicates.push(compactObject({
              originalFilename: file.originalname,
              storedFilename: file.filename,
              chksum: duplicateScan.checksum || null,
              duplicateTimestamp: duplicateJob.timestamp || null,
              duplicatePrinterId: duplicateJob.printerId || null,
              duplicatePrinterName: duplicateJob.printerName || null,
            }));
          ...
          filesToPrint.push({
            ...
            jobMeta: {
              ...
              ...(duplicateScan.checksum ? { chksum: duplicateScan.checksum } : {}),
            },
          });
        }
        ...
        if (fileKind === 'image' && printCopies > 1) {
          if (filesToPrint.length === 1) {
            ...
            logStamp(`Printing ${filesToPrint.length} image files with ${printCopies} copies each`);
          }
        }

        if (fileKind === 'image') {
          await printUploadedImages(
            req,
            filesToPrint,
            printerConfig,
            printCopies
          ...
          await Promise.all(filesToPrint.map(({ file, jobMeta }) => printUploadedFile(
            req,
            file,
            printerConfig,
            fileKind,
        ...
        res.status(200).json({
          ok: true,
          skippedDuplicates,
          printedCount: filesToPrint.length,
          skippedCount: skippedDuplicates.length,
        });
      } catch (error) {
    ...
    try {
      ...
      res.status(200).json({
        job: compactObject({
          ...
          storedFilename: originalJob.storedFilename || null,
    ...
    try {
      await Promise.all(Array.from({ length: normalizedCopyCount }, (_, index) => (
        printingService.printPDF(printJob.filePath, printerConfig, {
          ...
          chksum: printJob.chksum,
          isReprint: true,
          reprintSourceTimestamp: printJob.timestamp,
  ...
  app.post('/config', (req, res) => {
    try {
      ...
      fs.writeFileSync(path.join(rootDir, 'config.yaml'), rawConfig.endsWith('\n') ? rawConfig : `${rawConfig}\n`, 'utf8');
