...
const registerRoutes = ({
  ...
  app.post('/ingest/:sessionId/plugin-action', async (req, res) => {
    const sessionId = String(req.params.sessionId || '').trim();
    ...
    if (!action) {
      res.status(400).json({
        error: 'Missing plugin action',
      });
      return;
    }

    try {
      const result = await ingestService.confirmPendingPluginAction({
        sessionId,
        action,
      });
      ...
      if (error.code === 'INVALID_PLUGIN_ACTION') {
        res.status(error.status || 400).json({
          error: error.message || 'Unsupported pending plugin action',
        });
        return;
      }

      errorLogStamp('Pending plugin action failed:', error.message);
      res.status(500).json({
        error: 'Could not resolve pending plugin action',
      });
    }
  });
