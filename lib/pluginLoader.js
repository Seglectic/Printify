...
const createPluginManager = ({
  ...
  return {
    ...
    async inspectUpload(context = {}) {
      const pluginList = Object.values(plugins);

      for (const plugin of pluginList) {
        if (!isPluginActive(plugin) || typeof plugin.inspectUpload !== 'function') {
          continue;
        }

        try {
          const result = await plugin.inspectUpload(context);
          ...
          errorLogStamp(`Plugin "${plugin.id}" upload inspection failed:`, error.message);
        }
      }

      return null;
    },
    async resolvePendingUploadAction({ pluginId, ...context } = {}) {
      const plugin = plugins[String(pluginId || '').trim()] || null;

      if (!plugin || !isPluginActive(plugin) || typeof plugin.resolvePendingUploadAction !== 'function') {
        const error = new Error(`Upload action plugin "${pluginId}" is unavailable`);
        error.code = 'PLUGIN_ACTION_UNAVAILABLE';
        throw error;
      }

      return plugin.resolvePendingUploadAction(context);
    },
    isEnabled(pluginId) {
      const plugin = plugins[String(pluginId || '').trim()] || null;
